...
void ExecuteEmergencyClose()
{
   ...
   // 1. Collect all position tickets first so we don't block
   ulong posTickets[];
   ...
   // 2. Delete all pending limit orders instantly
   ...
   // 3. Fire lightning-fast asynchronous close orders directly to broker bridge
   MqlTick tick;
   ...
   for(int i = 0; i < posCount; i++)
   {
      ...
      // Fire asynchronously with zero wait!
      ...
      {
         // Fallback to synchronous close if async fails
         trade.PositionClose(ticket, InpSlippagePoints);
         fired++;
      }
   }

   PrintFormat("⚡ [LIGHTNING CLOSE FIRED]: %d closing requests broadcast instantly to broker!", fired);
}
